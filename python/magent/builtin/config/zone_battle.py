...
def get_config(map_size):
    ...
    small = cfg.register_agent_type(
        ...
        {'width': 1, 'length': 1, 'hp': 10, 'speed': 2,
         ...
         'step_reward': -0.005,  'kill_reward': 0, 'dead_penalty': -0.1, 'attack_penalty': -0.1,
         }) 
    ...
    cfg.add_reward_rule(gw.Event(a, 'attack', b), receiver=a, value=0.1)
    cfg.add_reward_rule(gw.Event(b, 'attack', a), receiver=b, value=0.1)
    
    zoneA = ((0, int(map_size//2 - map_size//10)), (int(map_size//5), int(map_size//2 + map_size//10)))
